 ...
 * unknown state never borrows an in-flight style. Rows, transcript, and
 * composer live in FleetSection; this file only mounts it.
 */
...
export function AgentsPanel({
  ...
  environmentId = null,
  threadId = null,
  fleetEnvironmentIds = null,
}: {
  model: AgentPanelModel;
  environmentId?: EnvironmentId | null;
  threadId?: ThreadId | null;
  /**
   * Connected environments whose native sessions appear in the fleet
   * section. Null (or empty) hides the section; the thread-local workflows
   * below are unchanged either way.
   */
  fleetEnvironmentIds?: ReadonlyArray<EnvironmentId> | null;
}) {
  const showFleet = fleetEnvironmentIds !== null && fleetEnvironmentIds.length > 0;
  if (!model.hasAgents && !showFleet) {
    return (
      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="flex flex-col gap-2 p-2">
          {showFleet && fleetEnvironmentIds !== null ? (
            <FleetSection environmentIds={fleetEnvironmentIds} />
          ) : null}
